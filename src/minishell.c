...
/*   Updated: 2024/08/13 10:47:39 by derey            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	loop_main(t_input line, t_execute *process, t_init *init_shell)
{
	...
	if (check_process(&tab_lexer, process, init_shell, &line) == 1)
	{
...
void	read_main(t_execute *process, t_input *line, t_init *init_shell)
{
	if (g_sig_exit != 0)
	{
		init_shell->exit_code = g_sig_exit;
		g_sig_exit = 0;
	}
...
int	main(int ac, char **av, char **env)
{
	...
	while (line.input && init_shell->on == 1)
		read_main(process, &line, init_shell);
	clear_history();
	all_unlink(init_shell->heredoc_name);
	ft_free(init_shell->cpy_env);
	free(init_shell->pathearly);
	free(init_shell->heredoc_name);
...
int	set_file(t_execute *process, t_pipe *pipes, t_lexer_tab *current, t_init *init_shell)
{
	...
	if (current->type == TOKEN_REDIRECT_INPUT)
	{
		...
		pipes[i].fdin_str = current->next->arg;
		if (pipes[i].fdin == -1)
		{
			current->next->infile = 2;
			return (-1);
	...
	{
		if (init_shell->heredoc_name)
			free(init_shell->heredoc_name);
		init_shell->heredoc_name = heredoc_name();
		if (heredoc(current, init_shell) != 0)
			return (-1);
	}
	if (current->type == TOKEN_ARGS)
	{
		...
		{	
			...
			pipes[i].fdin_str = current->next->arg + 1;
			if (pipes[i].fdin == -1)
			{
				current->next->infile = 2;
				return (-1);
...
void	pipeline_execute(t_input *line, t_execute *process, t_init *init_shell, t_lexer_tab *tab_lexer)
{
	...
	while (current)
	{
		...
		/*if (pipes[process->i].fdin_str)
			ft_printf("%s/n", pipes[process->i].fdin_str);*/
...
char	**get_command_args(t_input *line, t_lexer_tab **lexer_tab)
{
	...
	while (i < argc)
	{
		if (line->all_tokens[start_index + i][0] == '<'

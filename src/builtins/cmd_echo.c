...
static int	print_redir(t_input *line, int i)
{
	...
	while (line->input[a] != '\0')
	{
		...
		{
			if (line->input[a + 1] == '>' || line->input[a + 1] == '<')
				break ;
		}
		a++;
	}
	...
	{
		result++;
		if (line->all_tokens[result] != NULL
			&& line->all_tokens[result][0] == '.'
			&& line->all_tokens[result][1] == '/')
			result++;
	}
	return (result);
}
...
static void	print_echo(t_input *line, int i, int fd, int n)
{
	...
	while (line->all_tokens[i] != NULL && line->all_tokens[i][0] != '|')
	{
		...
		if (line->all_tokens[i][0] == '>' || line->all_tokens[i][0] == '<')
		{
